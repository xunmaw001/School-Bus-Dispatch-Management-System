		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 30px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","borderRadius":"3px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						...
						<el-input v-model="searchForm.jiashiyuangonghao" placeholder="驾驶员工号" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">报销项目</label>
						<el-input v-model="searchForm.baoxiaoxiangmu" placeholder="报销项目" clearable></el-input>
					</div>
				...
				<el-row :style='{"margin":"20px 0 20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(64, 158, 255, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('baoxiaoshenqing','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('baoxiaoshenqing','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				<el-table class="tables"
					...
					v-if="isAuth('baoxiaoshenqing','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="pingzheng" width="200" label="凭证">
						<template slot-scope="scope">
							<div v-if="scope.row.pingzheng">
								<img v-if="scope.row.pingzheng.substring(0,4)=='http'" :src="scope.row.pingzheng.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.pingzheng.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='true' v-if="isAuth('baoxiaoshenqing','审核')" prop="sfsh" label="审核">
						<template slot-scope="scope">
      ...
      let params = {
        ...
        order: 'desc',
          ...
          if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
           ...
           if(this.searchForm.baoxiaoxiangmu!='' && this.searchForm.baoxiaoxiangmu!=undefined){
            params['baoxiaoxiangmu'] = '%' + this.searchForm.baoxiaoxiangmu + '%'
          }
          if(this.searchForm.sfsh!='' && this.searchForm.sfsh!=undefined){
            params['sfsh'] = this.searchForm.sfsh
          }
    ...
    shDialog(row){
      ...
      if(row){
        this.shForm = {
          ...
          jiashiyuangonghao: row.jiashiyuangonghao,
          jiashiyuanxingming: row.jiashiyuanxingming,
          baoxiaoxiangmu: row.baoxiaoxiangmu,
          baoxiaojine: row.baoxiaojine,
          pingzheng: row.pingzheng,
