	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.xiangmubianhao" label="项目编号" prop="xiangmubianhao">
					<el-input v-model="ruleForm.xiangmubianhao" placeholder="项目编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="驾驶员工号" prop="jiashiyuangonghao">
					<el-input v-model="ruleForm.jiashiyuangonghao" placeholder="驾驶员工号" clearable  :readonly="ro.jiashiyuangonghao"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="驾驶员工号" prop="jiashiyuangonghao">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="报销项目" prop="baoxiaoxiangmu">
					<el-input v-model="ruleForm.baoxiaoxiangmu" placeholder="报销项目" clearable  :readonly="ro.baoxiaoxiangmu"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="报销项目" prop="baoxiaoxiangmu">
					<el-input v-model="ruleForm.baoxiaoxiangmu" placeholder="报销项目" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.pingzheng" label="凭证" prop="pingzheng">
					<file-upload
						tip="点击上传凭证"
						action="file/upload"
						:limit="3"
						:multiple="true"
						:fileUrls="ruleForm.pingzheng?ruleForm.pingzheng:''"
						@change="pingzhengUploadChange"
					></file-upload>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.pingzheng" label="凭证" prop="pingzheng">
					<img v-if="ruleForm.pingzheng.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.pingzheng.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.pingzheng.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='baoxiaoxiangmu'){
							this.ruleForm.baoxiaoxiangmu = obj[o];
							this.ro.baoxiaoxiangmu = true;
							continue;
						}
						if(o=='baoxiaojine'){
						...
						if(o=='pingzheng'){
							this.ruleForm.pingzheng = obj[o];
							this.ro.pingzheng = true;
							continue;
						}
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					var json = data.data;
					if(((json.jiashiyuangonghao!=''&&json.jiashiyuangonghao) || json.jiashiyuangonghao==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.jiashiyuangonghao = json.jiashiyuangonghao
						this.ro.jiashiyuangonghao = true;
					}
					if(((json.jiashiyuanxingming!=''&&json.jiashiyuanxingming) || json.jiashiyuanxingming==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.jiashiyuanxingming = json.jiashiyuanxingming
						this.ro.jiashiyuanxingming = true;
					}
				} else {
	...
	if(this.ruleForm.pingzheng!=null) {
		this.ruleForm.pingzheng = this.ruleForm.pingzheng.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `baoxiaoshenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.baoxiaoshenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    pingzhengUploadChange(fileUrls) {
	    this.ruleForm.pingzheng = fileUrls;
    },
  }
};
</script>
