		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 30px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","borderRadius":"3px","background":"#fff","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">借调公司</label>
						<el-input v-model="searchForm.jiediaogongsi" placeholder="借调公司" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="借调车辆" prop="jiediaocheliang">
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"500","height":"40px"}' class="item-label">借调车辆</label>
						<el-select  @change="jiediaocheliangChange" clearable v-model="searchForm.jiediaocheliang" placeholder="请选择借调车辆">
							<el-option v-for="(item,index) in jiediaocheliangOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row :style='{"margin":"20px 0 20px 0","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(64, 158, 255, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('jiediaocheliang','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"rgba(255, 0, 0, 1)","width":"auto","fontSize":"14px","height":"40px"}' v-if="isAuth('jiediaocheliang','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				<el-table class="tables"
					...
					v-if="isAuth('jiediaocheliang','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					label="借调公司">
						<template slot-scope="scope">
							{{scope.row.jiediaogongsi}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiediaocheliang"
					label="借调车辆">
						<template slot-scope="scope">
							{{scope.row.jiediaocheliang}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="jiediaoshijian"
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.jiediaogongsi!='' && this.searchForm.jiediaogongsi!=undefined){
            params['jiediaogongsi'] = '%' + this.searchForm.jiediaogongsi + '%'
          }
           if(this.searchForm.jiediaocheliang!='' && this.searchForm.jiediaocheliang!=undefined){
            params['jiediaocheliang'] = this.searchForm.jiediaocheliang
          }
    params['sort'] = 'jiediaoshijian';
    params['order'] = 'desc';
